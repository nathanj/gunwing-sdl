...
{
	position_ = {x, y};
	direction_ = { rand_between(-100, 100) / 100.0f, rand_between(-100, 100) / 100.0f };
	direction_.normalize();
	speed_ = rand()%3 + 1;

	geometry_.push_back( {0, 0, static_cast<float>(image_.w), static_cast<float>(image_.h)} );
}
...
void Medal::update(std::chrono::milliseconds delta)
{
	...
	if (!delay_time_.active()) {
		Vector<float> newdir = {
			(GameState::ship->position().x + Ship::image_.w / 2 - image_.w / 2) - position_.x,
			(GameState::ship->position().y + Ship::image_.h / 2 - image_.h / 2) - position_.y
		};
		newdir.normalize();
		...
		// medal to the ship.
		Vector<float> distance = GameState::ship->position() - position_;
...
{
	graphics.blit(image_, 0, 0, position_.x, position_.y, image_.w, image_.h);
}
